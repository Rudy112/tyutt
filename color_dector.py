# -*- coding: utf-8 -*-
"""
Created on Thu Dec  9 13:48:11 2021
...
dire = "E://cloth_datasets//cloth_datasets//clean_datasets//"
dire = "E://cloth_datasets//cloth_datasets//"
filename =  os.listdir(dire)
#filename = sorted(filename, key = index) 
pngs = [x for x in filename if (x.endswith(".jpg"))]
pcds = [x for x in filename if x.endswith(".npy")]
for i in range(len(pngs)):#[]range(1):#len(jpgs)):
	print(i)
	print(pngs[i])
	img_rgb = cv2.imread(dire+pngs[i])
	#row_start = 150
	#row_end = 660
	#col_start = 215
	#col_end = 1100
...
# 	cv2.imwrite('./test_color/img_rgb.png',(img_rgb).astype(np.uint8))
	...
	cv2.imwrite(dire+(pngs[i].split("_")[1])+'_labels_red.png', labels_red)
	print(dire+(pngs[i].split("_")[1])+'_labels_red.png')
	cv2.imwrite(dire+(pngs[i].split("_")[1])+'_labels_yellow.png',labels_yellow)
	cv2.imwrite(dire+(pngs[i].split("_")[1])+'_labels_purple.png', labels_purple)
	cv2.imwrite(dire+(pngs[i].split("_")[1])+'_labels_blue.png', labels_blue)
	cv2.imwrite(dire+(pngs[i].split("_")[1])+'_labels_green.png', labels_green)
	print(dire+(pngs[i].split("_")[1])+'_labels_green.png')
	cv2.imwrite(dire+(pngs[i].split("_")[1])+'_labels_orange.png', labels_orange)
